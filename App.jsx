...
function LoginScreen({ onLogin, onSendMagicLink, loading, erro }) {
  ...
  return (
    <div className="login-shell">
      <section className="login-left">
        ...
        <h1>Login profissional para sua ótica</h1>
        <p className="login-description">
          Entre com email e senha do Supabase Auth. Se preferir, envie um link mágico para o email autorizado.
        </p>
        <div className="login-badges">
          <span>Supabase Auth</span>
          <span>Vínculo por empresa</span>
      ...
      <form className="login-card" onSubmit={submit}>
        ...
        <p className="dark-text">Use um email autorizado e sua senha.</p>
        ...
        <button className="secondary-btn" type="button" onClick={sendMagicLink} disabled={loading || !email.trim()}>
          Receber link mágico
        </button>
      </form>
    </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      await carregarEmpresaPorEmail(session.user.email)
    }
  ...
  useEffect(() => {
    if (!empresaAtual?.codempresa) return

    fetchAll(empresaAtual.codempresa)
    const refresh = () => fetchAll(empresaAtual.codempresa)
    const channels = [
      supabase.channel(`lancamentos-realtime-${empresaAtual.codempresa}`).on('postgres_changes', { event: '*', schema: 'public', table: 'lancamentos_caixa' }, refresh).subscribe(),
      supabase.channel(`aberturas-realtime-${empresaAtual.codempresa}`).on('postgres_changes', { event: '*', schema: 'public', table: 'aberturas_caixa' }, refresh).subscribe(),
      supabase.channel(`fechamentos-realtime-${empresaAtual.codempresa}`).on('postgres_changes', { event: '*', schema: 'public', table: 'fechamentos_caixa' }, refresh).subscribe(),
      supabase.channel('clientes-realtime').on('postgres_changes', { event: '*', schema: 'public', table: 'clientes' }, refresh).subscribe(),
      supabase.channel('cliente-exames-realtime').on('postgres_changes', { event: '*', schema: 'public', table: 'cliente_exames' }, refresh).subscribe(),
      supabase.channel('cliente-financeiro-realtime').on('postgres_changes', { event: '*', schema: 'public', table: 'cliente_financeiro' }, refresh).subscribe()
    ]
  ...
  async function carregarEmpresaPorEmail(email) {
    const emailNormalizado = normalizeEmail(email)
    setErro('')

    const { data: acessos, error: acessoError } = await supabase
      .from('acessos_empresa')
      .select('email, ativo, perfil, empresas:codempresa (codempresa, nome, email_acesso, ativo)')
      .eq('email', emailNormalizado)
      .eq('ativo', true)
      .maybeSingle()

    if (!acessoError && acessos?.empresas?.codempresa) {
      const empresa = acessos.empresas
    ...
    if (!empresaEncontrada) {
      setEmpresaAtual(null)
      setErro('Seu usuário autenticado não está vinculado a nenhuma ótica. Cadastre o email na tabela acessos_empresa ou em empresas.email_acesso.')
      setLoading(false)
      return null
    }
  ...
  async function handleLogin(emailDigitado, senhaDigitada) {
    ...
    if (error) {
      setErro(`Login inválido. ${error.message}`)
      setAuthLoading(false)
      return
    }

    setCurrentUser(data?.user || null)
    await carregarEmpresaPorEmail(emailNormalizado)
    setAuthLoading(false)
  }

  async function enviarMagicLink(emailDigitado) {
    const emailNormalizado = normalizeEmail(emailDigitado)

    if (!emailNormalizado) {
      setErro('Informe seu email para receber o link.')
      return
    }
    ...
    if (error) {
      setErro(`Não foi possível enviar o link mágico. ${error.message}`)
      setAuthLoading(false)
      return
    }

    setErro('Enviamos um link de acesso para seu email.')
    setAuthLoading(false)
  }
  ...
  async function submitForm(e) {
    ...
    const { error } = await supabase.from('lancamentos_caixa').insert([{ data: form.data, descricao: form.descricao.trim(), categoria: form.categoria, tipo: form.tipo, forma: form.forma, valor, observacao: (form.observacao || '').trim(), responsavel: form.responsavel, codempresa: empresaAtual.codempresa }])
    if (error) { setErro(`Não foi possível salvar o lançamento. ${error.message}`); setSalvando(false); return }
    setForm(emptyForm(selectedDate, empresaAtual.codempresa)); setSalvando(false); fetchAll(empresaAtual?.codempresa)
  }
  ...
  async function confirmarAbertura() {
    ...
    setAbrindo(true)
    const { error } = await supabase.from('aberturas_caixa').insert([{ data: selectedDate, valor_inicial: valorInicial, aberto_por: aberturaPor || 'MEIRE', observacao: (aberturaObs || '').trim(), codempresa: empresaAtual.codempresa }])
    if (error) { setErro(`Não foi possível abrir o caixa. ${error.message}`); setAbrindo(false); return }
    setAbrindo(false); setShowAberturaModal(false); setAberturaValor(''); setAberturaObs(''); fetchAll(empresaAtual?.codempresa)
  }
  ...
  async function confirmarFechamento() {
    ...
    setFechando(true)
    const { error } = await supabase.from('fechamentos_caixa').insert([{ data: selectedDate, total_entradas: totalEntradas, total_saidas: totalSaidas, saldo: saldoDia, fechado_por: fechamentoPor, observacao: (fechamentoObs || '').trim(), codempresa: empresaAtual.codempresa }])
    if (error) { setErro(`Não foi possível fechar o caixa. ${error.message}`); setFechando(false); return }
    setFechando(false); setShowFechamentoModal(false); setFechamentoObs(''); fetchAll(empresaAtual?.codempresa)
  }

  if (!empresaAtual) {
    return <LoginScreen onLogin={handleLogin} onSendMagicLink={enviarMagicLink} loading={authLoading} erro={erro} />
  }
