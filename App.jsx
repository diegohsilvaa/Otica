...
function emptyForm(date) {
  return { data: date || getTodayLocal(), descricao: '', categoria: 'Venda', tipo: 'Entrada', forma: 'Pix', valor: '', observacao: '', responsavel: 'MEIRE' }
}
    ...
    <div className="welcome-shell">
      <div className="welcome-content">
        <div className="welcome-brand">
          <div className="welcome-logo">ÓF</div>
          <p className="welcome-tag">Sistema interno</p>
          <h1>ÓTICA FÁCIL</h1>
          <span className="welcome-city">AGRESTINA - PE</span>
        </div>

        <div className="welcome-card">
          <p className="welcome-kicker">Bem-vindo</p>
          <h2>Controle de caixa e cadastro de clientes</h2>
          <p className="welcome-text">Abertura inicial bonita e profissional para começar o atendimento com organização.</p>

          <div className="welcome-info-grid">
            <div className="welcome-info-item"><small>Empresa</small><strong>Ótica Fácil</strong></div>
            <div className="welcome-info-item"><small>Cidade</small><strong>Agrestina - PE</strong></div>
            <div className="welcome-info-item"><small>Endereço</small><strong>Rua João de Deus, nº 292</strong></div>
            <div className="welcome-info-item"><small>Telefone</small><strong>81 99748-6190</strong></div>
          </div>

          <div className="welcome-actions">
            <button type="button" className="welcome-btn" onClick={onEnter}>Entrar no sistema</button>
          </div>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const [abaAtual, setAbaAtual] = useState('livro-caixa')
  ...
  async function fetchAll() {
    ...
    if (lancResp.error || abResp.error || fechResp.error || cliResp.error) {
      setErro(lancResp.error?.message || abResp.error?.message || fechResp.error?.message || cliResp.error?.message || 'Erro ao carregar dados.')
      setLoading(false)
      return
    }
  ...
  const resumoCategorias = useMemo(() => {
    ...
    lancamentosExibidos.forEach(item => {
      ...
      if (!mapa[chave]) mapa[chave] = { chave, tipo: item.tipo, categoria: item.categoria, quantidade: 0, valor: 0, itens: [] }
  ...
  async function submitForm(e) {
    ...
    const { error } = await supabase.from('lancamentos_caixa').insert([{
      data: form.data, descricao: form.descricao.trim(), categoria: form.categoria, tipo: form.tipo, forma: form.forma, valor,
      observacao: (form.observacao || '').trim(), responsavel: form.responsavel
    }])
    setSalvando(false)
  ...
  async function submitCliente(e) {
    e.preventDefault()
    setErro('')
    if (!clienteForm.codigo.trim() || !clienteForm.nome.trim()) return setErro('Preencha código e nome do cliente.')
    setSalvandoCliente(true)
    const { error } = await supabase.from('clientes').insert([{
      codigo: clienteForm.codigo.trim(), nome: clienteForm.nome.trim(), contato: (clienteForm.contato || '').trim(),
      endereco: (clienteForm.endereco || '').trim(), numero: (clienteForm.numero || '').trim(),
      complemento: (clienteForm.complemento || '').trim(), bairro: (clienteForm.bairro || '').trim(),
      cidade: (clienteForm.cidade || '').trim(), uf: (clienteForm.uf || '').trim(), cep: (clienteForm.cep || '').trim()
    }])
  ...
  async function confirmarAbertura() {
    ...
    setAbrindo(true)
    const { error } = await supabase.from('aberturas_caixa').insert([{ data: selectedDate, valor_inicial: valorInicial, aberto_por: aberturaPor, observacao: (aberturaObs || '').trim() }])
    setAbrindo(false)
  ...
  async function confirmarFechamento() {
    ...
    if (fechamentoDoDia) return setErro('Este dia já foi fechado.')
    setFechando(true)
    const { error } = await supabase.from('fechamentos_caixa').insert([{ data: selectedDate, total_entradas: entradasDia, total_saidas: saidasDia, saldo: saldoDia, fechado_por: fechamentoPor, observacao: (fechamentoObs || '').trim() }])
    setFechando(false)
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="menu-card">
          <button type="button" className={abaAtual === 'livro-caixa' ? 'menu-item active menu-button' : 'menu-item menu-button'} onClick={() => setAbaAtual('livro-caixa')}>Livro Caixa</button>
      ...
      <main className="content">
        ...
        {abaAtual === 'livro-caixa' ? (
          <>
            ...
            <section className="panel">
              <div className="panel-title-row"><div><h3>Fluxo do caixa</h3><p className="topbar-subtitle">Abra, acompanhe e feche o caixa do dia.</p></div></div>
              {!aberturaDoDia ? (
                <button className="secondary-btn" type="button" onClick={() => setShowAberturaModal(true)}>Abrir caixa do dia</button>
              ) : fechamentoDoDia ? (
                <div className="opened-info"><strong>Aberto por {aberturaDoDia.aberto_por}</strong><span>Fechado por {fechamentoDoDia.fechado_por} • saldo final {formatMoney(fechamentoDoDia.saldo)}</span></div>
              ) : (
                <div className="opened-info"><strong>Caixa aberto por {aberturaDoDia.aberto_por}</strong><span>Valor inicial {formatMoney(aberturaDoDia.valor_inicial)}</span><button className="secondary-btn" type="button" onClick={() => setShowFechamentoModal(true)}>Revisar e fechar o dia</button></div>
              )}
            </section>

            <section className="main-grid">
              <form className="panel featured-form" onSubmit={submitForm}>
                <div className="panel-head"><div><span className="badge-title">Área principal</span><h3>Novo lançamento financeiro</h3><p>É aqui que você registra recebimentos, entradas e saídas do caixa.</p></div></div>
              ...
              <section className="panel">
                <div className="panel-head">
                  <div><h3>Livro caixa</h3><p>Veja de forma sintética ou analítica.</p></div>
                    ...
                    {resumoCategorias.length === 0 ? <div className="empty-state">Nenhum lançamento encontrado.</div> : resumoCategorias.map(grupo => (
                      <article className="summary-card" key={grupo.chave}>
                        <div className="summary-header">
                          <div><div className="summary-title-row"><h4>{grupo.categoria}</h4><span className={grupo.tipo === 'Entrada' ? 'pill in' : 'pill out'}>{grupo.tipo}</span></div><p>{grupo.quantidade} lançamento(s)</p></div>
                  ...
                  <div className="launch-list">
                    {lancamentosExibidos.length === 0 ? <div className="empty-state">Nenhum lançamento encontrado.</div> : lancamentosExibidos.map(item => (
                      <article className="launch-card" key={item.id}>
                        <div className="launch-top"><div><div className="launch-title">{item.descricao}</div><div className="launch-meta">{formatDate(item.data)} • {item.categoria} • {item.forma}</div></div><div className={item.tipo === 'Entrada' ? 'pill in' : 'pill out'}>{item.tipo}</div></div>
                        <div className="launch-bottom"><div><div className="launch-value">{formatMoney(item.valor)}</div><div className="launch-note">{item.observacao || 'Sem observação'} • Lançado por: {item.responsavel}</div></div></div>
                      </article>
                    ))}
                  </div>
                )}
          ...
          <>
            <header className="topbar">
              <div><p className="page-kicker">Cadastro</p><h2>Clientes</h2><p className="topbar-subtitle">Ficha mais completa com contato e endereço.</p></div>
            </header>
            <section className="main-grid clientes-grid">
              <form className="panel featured-form" onSubmit={submitCliente}>
                <div className="panel-head"><div><span className="badge-title">Novo cadastro</span><h3>Cadastrar cliente</h3><p>Agora com dados de localização para montar melhor a ficha do cliente.</p></div></div>
                <div className="form-grid">
                  <label className="field"><span>Código</span><input value={clienteForm.codigo} onChange={(e) => updateClienteField('codigo', e.target.value)} placeholder="Ex.: 0001" /></label>
              ...
              <section className="panel">
                <div className="panel-head"><div><h3>Lista de clientes</h3><p>Veja os dados principais e o endereço resumido.</p></div></div>
                <div className="filters"><input placeholder="Buscar por código, nome, contato ou endereço" value={buscaCliente} onChange={(e) => setBuscaCliente(e.target.value)} /></div>
        ...
        {showAberturaModal ? <div className="modal-overlay"><div className="modal-card"><div className="panel-head"><h3>Abrir caixa</h3><button type="button" className="secondary-btn" onClick={() => setShowAberturaModal(false)}>Fechar</button></div><div className="form-grid"><label className="field"><span>Responsável</span><select value={aberturaPor} onChange={(e) => setAberturaPor(e.target.value)}>{responsaveis.map(item => <option key={item}>{item}</option>)}</select></label><label className="field"><span>Valor inicial</span><input value={aberturaValor} onChange={(e) => setAberturaValor(e.target.value)} placeholder="Ex.: 200,00" /></label><label className="field field-full"><span>Observação</span><textarea rows="3" value={aberturaObs} onChange={(e) => setAberturaObs(e.target.value)} /></label></div><div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setShowAberturaModal(false)}>Cancelar</button><button type="button" className="primary-inline-btn" onClick={confirmarAbertura} disabled={abrindo}>{abrindo ? 'Abrindo...' : 'Confirmar abertura'}</button></div></div></div> : null}

        {showFechamentoModal ? <div className="modal-overlay"><div className="modal-card"><div className="panel-head"><h3>Fechar caixa</h3><button type="button" className="secondary-btn" onClick={() => setShowFechamentoModal(false)}>Fechar</button></div><div className="stats-grid" style={{marginBottom:16}}><div className="stat-card"><span className="small-label">Inicial</span><strong>{formatMoney(valorInicialDia)}</strong></div><div className="stat-card"><span className="small-label">Entradas</span><strong className="in-text">{formatMoney(entradasDia)}</strong></div><div className="stat-card"><span className="small-label">Saídas</span><strong className="out-text">{formatMoney(saidasDia)}</strong></div><div className="stat-card"><span className="small-label">Saldo</span><strong>{formatMoney(saldoDia)}</strong></div></div><div className="form-grid"><label className="field"><span>Responsável</span><select value={fechamentoPor} onChange={(e) => setFechamentoPor(e.target.value)}>{responsaveis.map(item => <option key={item}>{item}</option>)}</select></label><label className="field field-full"><span>Observação</span><textarea rows="3" value={fechamentoObs} onChange={(e) => setFechamentoObs(e.target.value)} /></label></div><div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setShowFechamentoModal(false)}>Cancelar</button><button type="button" className="primary-inline-btn" onClick={confirmarFechamento} disabled={fechando}>{fechando ? 'Fechando...' : 'Confirmar fechamento'}</button></div></div></div> : null}
      </main>
    </div>
  )
}
