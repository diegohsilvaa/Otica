...
function getFinanceiroStatus(item) {
  const total = Number(item?.valor_total || 0)
  const quitado = Number(item?.valor_quitado || 0)
  if (quitado <= 0) return 'Em aberto'
  if (quitado >= total && total > 0) return 'Pago'
  return 'Parcial'
}

function getFinanceiroProgress(item) {
  const total = Number(item?.valor_total || 0)
  const quitado = Number(item?.valor_quitado || 0)
  if (!total || total <= 0) return 0
  return Math.max(0, Math.min(100, Math.round((quitado / total) * 100)))
}
...
export default function App() {
  ...
  useEffect(() => {
    if (financeiroForm.tipo_cobranca !== 'carne' && financeiroForm.quantidade_parcelas !== '1') {
      setFinanceiroForm((prev) => ({ ...prev, quantidade_parcelas: '1' }))
    }
  }, [financeiroForm.tipo_cobranca])
  ...
  const financeiroDoClienteOrdenado = useMemo(() => [...financeiroDoCliente].sort((a, b) => {
    const grupoA = String(a.grupo_parcelas || a.id || '')
    const grupoB = String(b.grupo_parcelas || b.id || '')
    if (grupoA === grupoB) {
      return Number(a.numero_parcela || 1) - Number(b.numero_parcela || 1)
    }
    const emissaoA = String(a.emissao || '')
    const emissaoB = String(b.emissao || '')
    return emissaoB.localeCompare(emissaoA)
  }), [financeiroDoCliente])
...
async function salvarFinanceiro(e) {
  ...
  const statusAtual = valorQuitado <= 0 ? 'Em aberto' : valorQuitado >= valorTotal ? 'Pago' : 'Parcial'
  ...
  const basePayload = {
    ...
    valor_quitado: Math.min(valorQuitado, valorTotal),
    data_pagamento: valorQuitado > 0 ? (financeiroForm.data_pagamento || getTodayLocal()) : null,
    forma_pagamento: financeiroForm.forma_pagamento.trim(),
    observacao: financeiroForm.observacao.trim(),
    tipo_cobranca: tipoCobranca,
    quantidade_parcelas: tipoCobranca === 'carne' ? quantidadeParcelas : 1,
  ...
  if (financeiroForm.id) {
    ...
    const parcelas = Array.from({ length: quantidadeParcelas }, (_, index) => {
      ...
      const ajusteFinal = numeroParcela === quantidadeParcelas ? diferenca : 0
      const valorParcela = Number((valorParcelaBase + ajusteFinal).toFixed(2))
      return {
        ...basePayload,
        ...
        valor_quitado: 0,
        data_pagamento: null,
        vencimento: financeiroForm.vencimento ? addDays(financeiroForm.vencimento, index * 30) : null,
    ...
    const response = await supabase.from('cliente_financeiro').insert([{
      ...basePayload,
      grupo_parcelas: basePayload.grupo_parcelas || `LANC-${Date.now()}`,
      numero_parcela: 1,
      quantidade_parcelas: 1
    }])
    error = response.error
...
async function registrarBaixaFinanceiro(item) {
  const valorRestante = Math.max(0, Number(item.valor_total || 0) - Number(item.valor_quitado || 0))
  if (valorRestante <= 0) return

  const entrada = window.prompt(`Valor recebido desta parcela:
Restante: ${formatMoney(valorRestante)}`, String(valorRestante).replace('.', ','))
  if (entrada === null) return

  const valorRecebido = parseMoney(entrada)
  if (!Number.isFinite(valorRecebido) || valorRecebido <= 0) {
    setErro('Informe um valor válido para a baixa.')
    return
  }

  const novoQuitado = Math.min(Number(item.valor_quitado || 0) + valorRecebido, Number(item.valor_total || 0))
  const total = Number(item.valor_total || 0)
  const novoStatus = novoQuitado <= 0 ? 'Em aberto' : novoQuitado >= total ? 'Pago' : 'Parcial'

  const { error } = await supabase.from('cliente_financeiro').update({
    valor_quitado: Number(novoQuitado.toFixed(2)),
              ...
              <div className="client-tab-content">
                {clienteTab === 'dados' ? (
                  ...
                  <div className="client-scroll-area">
                    ...
                    <form className="panel nested-panel finance-form-panel" onSubmit={salvarFinanceiro}>
                      <div className="panel-head">
                        <div>
                          <h3>Financeiro do cliente</h3>
                          <p>Carnê repete as parcelas automaticamente a cada 30 dias. Cartão, débito e dinheiro ficam em uma única linha.</p>
                        </div>
                      </div>

                      <div className="finance-form-sections">
                        <div className="finance-section-card">
                          ...
                          <div className="form-grid">
                            ...
                            <label className="field"><span>Duplicata / descrição</span><input value={financeiroForm.duplicata} onChange={(e) => updateFinanceiroField('duplicata', e.target.value)} placeholder="Ex.: Carnê armação, Cartão, Dinheiro" /></label>
                            <label className="field"><span>Tipo de cobrança</span><select value={financeiroForm.tipo_cobranca} onChange={(e) => updateFinanceiroField('tipo_cobranca', e.target.value)}><option value="carne">Carnê</option><option value="cartao_credito">Cartão de crédito</option><option value="cartao_debito">Cartão de débito</option><option value="dinheiro">Dinheiro</option><option value="pix">Pix</option><option value="boleto">Boleto</option></select></label>
                            <label className="field"><span>Forma de pagamento</span><input value={financeiroForm.forma_pagamento} onChange={(e) => updateFinanceiroField('forma_pagamento', e.target.value)} placeholder="Ex.: Carnê, Crédito, Débito, Dinheiro" /></label>
                          </div>
                        </div>

                        <div className="finance-section-card">
                          <h4>Parcelas e recebimento</h4>
                          <div className="form-grid">
                            <label className="field">
                              <span>Qtd. parcelas</span>
                              <input type="number" min="1" value={financeiroForm.quantidade_parcelas} onChange={(e) => updateFinanceiroField('quantidade_parcelas', e.target.value)} disabled={financeiroForm.tipo_cobranca !== 'carne'} />
                              <small className="field-help">{financeiroForm.tipo_cobranca === 'carne' ? 'Se tiver mais de 1 parcela, as próximas vencem de 30 em 30 dias.' : 'Para esta cobrança o sistema lança somente uma linha.'}</small>
                            </label>
                            ...
                            <label className="field"><span>Valor já recebido</span><input value={financeiroForm.valor_quitado} onChange={(e) => updateFinanceiroField('valor_quitado', e.target.value)} /></label>
                            <label className="field"><span>Data do pagamento</span><input type="date" value={financeiroForm.data_pagamento} onChange={(e) => updateFinanceiroField('data_pagamento', e.target.value)} /></label>
                            <label className="field"><span>Status</span><select value={financeiroForm.status} onChange={(e) => updateFinanceiroField('status', e.target.value)}><option>Em aberto</option><option>Parcial</option><option>Pago</option></select></label>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="finance-preview-note">
                        {financeiroForm.tipo_cobranca === 'carne'
                          ? `Ao salvar, o carnê será dividido em ${Math.max(1, Number(financeiroForm.quantidade_parcelas || 1))} parcela(s).`
                          : 'Ao salvar, será gerado apenas um lançamento financeiro.'}
                      </div>

                      <div className="modal-actions sticky-actions">
                        <button type="button" className="secondary-btn" onClick={() => setFinanceiroForm(emptyFinanceiro())}>Limpar</button>
                        <button type="submit" className="primary-inline-btn" disabled={salvandoFinanceiro}>{salvandoFinanceiro ? 'Salvando...' : financeiroForm.id ? 'Salvar alteração do financeiro' : 'Adicionar financeiro'}</button>
                      </div>
                    </form>
                    ...
                    <div className="panel nested-panel">
                      <div className="panel-head">
                        <div>
                          ...
                          <p>Agora organizada no padrão do sistema, com visual claro para parcela em aberto, parcial e paga.</p>
                        </div>
                      </div>

                      {financeiroDoClienteOrdenado.length === 0 ? <div className="empty-state">Nenhum lançamento financeiro para este cliente.</div> : (
                        <div className="finance-ledger">
                          ...
                          <div className="finance-ledger-body">
                            {financeiroDoClienteOrdenado.map((item) => {
                              const status = getFinanceiroStatus(item)
                              const progresso = getFinanceiroProgress(item)
                              const statusSlug = status.toLowerCase().replace(' ', '-')
                              const parcelaLabel = item.tipo_cobranca === 'carne' ? `${item.numero_parcela || 1}/${item.quantidade_parcelas || 1}` : 'Única'
                              const tipoLabel = item.tipo_cobranca === 'carne'
                                ? 'Carnê'
                                : item.tipo_cobranca === 'cartao_credito'
                                  ? 'Cartão crédito'
                                  : item.tipo_cobranca === 'cartao_debito'
                                    ? 'Cartão débito'
                                    : item.tipo_cobranca === 'dinheiro'
                                      ? 'Dinheiro'
                                      : item.tipo_cobranca === 'pix'
                                        ? 'Pix'
                                        : 'Boleto'

                              return (
                                <article key={item.id} className={`finance-ledger-row status-${statusSlug}`}>
                                  <div className="finance-cell">
                                    <span className="finance-mobile-label">Emissão</span>
                                    <strong>{formatDate(item.emissao)}</strong>
                                  </div>

                                  <div className="finance-cell finance-cell-dup">
                                    <span className="finance-mobile-label">Duplicata</span>
                                    <strong>{item.duplicata || 'Lançamento'}</strong>
                                    {item.observacao ? <small>{item.observacao}</small> : <small>Sem observação</small>}
                                  </div>

                                  <div className="finance-cell">
                                    <span className="finance-mobile-label">Tipo</span>
                                    <strong>{tipoLabel}</strong>
                                  </div>

                                  <div className="finance-cell">
                                    <span className="finance-mobile-label">Parcela</span>
                                    <strong>{parcelaLabel}</strong>
                                  </div>

                                  <div className="finance-cell">
                                    <span className="finance-mobile-label">Vencimento</span>
                                    <strong>{formatDate(item.vencimento)}</strong>
                                  </div>

                                  <div className="finance-cell">
                                    <span className="finance-mobile-label">Valor</span>
                                    <strong>{formatMoney(item.valor_total)}</strong>
                                  </div>

                                  <div className="finance-cell finance-cell-paid">
                                    <span className="finance-mobile-label">Recebido</span>
                                    <strong>{formatMoney(item.valor_quitado)}</strong>
                                    <small>{item.data_pagamento ? `Último pgto: ${formatDate(item.data_pagamento)}` : 'Sem pagamento'}</small>
                                    <div className="finance-progress-track">
                                      <div className={`finance-progress-fill fill-${statusSlug}`} style={{ width: `${progresso}%` }} />
                                    </div>
                                    <small>{progresso}% recebido</small>
                                  </div>

                                  <div className="finance-cell finance-cell-actions">
                                    <span className={`status-badge badge-${statusSlug}`}>{status}</span>
                                    <div className="finance-actions">
                                      {status === 'Pago'
                                        ? <button className="secondary-compact-btn" type="button" onClick={() => removerBaixaFinanceiro(item)}>Reabrir</button>
                                        : <button className="primary-inline-btn" type="button" onClick={() => registrarBaixaFinanceiro(item)}>{status === 'Parcial' ? 'Receber restante' : 'Dar baixa'}</button>}
