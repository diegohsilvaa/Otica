                          ...
                          <table className="financeiro-table">
                            ...
                            <tbody>
                              {financeiroDoCliente.map((item) => {
                                ...
                                return (
                                  <tr key={item.id} className={`financeiro-status-row status-${status.toLowerCase().replace(' ', '-')}`}>
                                    <td>{formatDate(item.emissao)}</td>
                                    <td>
                                      <div className="finance-dup">
                                        <strong>{item.duplicata || 'Lançamento'}</strong>
                                        {item.observacao ? <small>{item.observacao}</small> : null}
                                      </div>
                                    </td>
                                    <td>{item.tipo_cobranca === 'carne' ? 'Carnê' : item.tipo_cobranca === 'cartao_credito' ? 'Cartão crédito' : item.tipo_cobranca === 'cartao_debito' ? 'Cartão débito' : item.tipo_cobranca === 'dinheiro' ? 'Dinheiro' : item.tipo_cobranca === 'pix' ? 'Pix' : 'Boleto'}</td>
                                    <td>{parcelaLabel}</td>
                                    <td>{formatDate(item.vencimento)}</td>
                                    <td>{formatMoney(item.valor_total)}</td>
                                    <td>
                                      <div className="finance-paid-cell">
                                        <strong>{formatMoney(item.valor_quitado)}</strong>
                                        <small>{formatDate(item.data_pagamento)}</small>
                                      </div>
                                    </td>
                                    <td>
                                      <div className="finance-progress-cell">
                                        <div className="finance-progress-track">
                                          <div className={`finance-progress-fill fill-${status.toLowerCase().replace(' ', '-')}`} style={{ width: `${progresso}%` }} />
                                        </div>
                                        <small>{progresso}% recebido</small>
                                      </div>
                                    </td>
                                    <td><span className={`status-badge badge-${status.toLowerCase().replace(' ', '-')}`}>{status}</span></td>
                                    <td>
