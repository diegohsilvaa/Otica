    ...
    <div className="login-shell">
      <section className="login-left">
        ...
        <p className="login-description">
          Entre com o email e a senha cadastrados para a sua empresa. Cada login acessa apenas os dados da ótica vinculada.
        </p>
        <div className="login-badges">
          <span>Empresa por login</span>
      ...
      <form className="login-card" onSubmit={submit}>
        ...
        <p className="dark-text">Informe o email e a senha cadastrados na tabela de usuários.</p>
...
export default function App() {
  ...
  useEffect(() => {
    const persisted = localStorage.getItem('otica_usuario')
    if (!persisted) {
      setLoading(false)
      return
    }
    try {
      const usuario = JSON.parse(persisted)
      if (usuario?.codempresa) {
        setUsuarioAtual(usuario)
        setEmpresaAtual({ codempresa: usuario.codempresa, nome: usuario.empresa_nome || usuario.nome_empresa || 'Ótica' })
        setForm(emptyForm(hoje, usuario.codempresa))
      } else {
  ...
  async function handleLogin(emailDigitado, senhaDigitada) {
    ...
    const senhaNormalizada = String(senhaDigitada || '')
    ...
    if (!usuario?.codempresa) {
      setErro('Email ou senha inválidos, ou usuário sem empresa vinculada.')
      setAuthLoading(false)
      return
    }
    ...
    localStorage.setItem('otica_usuario', JSON.stringify(sessao))
    setUsuarioAtual(sessao)
    setEmpresaAtual({ codempresa: usuario.codempresa, nome: usuario.empresa_nome || 'Ótica' })
    setForm(emptyForm(hoje, usuario.codempresa))
    ...
    <div className="app-shell">
      <aside className="sidebar">
        <div className="brand-card">
          ...
          {usuarioAtual?.email ? <p className="login-description" style={{ marginTop: 12 }}>{usuarioAtual.email}</p> : null}
        </div>
