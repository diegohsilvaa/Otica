...
function emptyExame() {
  return {
    id: null,
    data_exame: getTodayLocal(),
    profissional: '',
    receita: '',
    od_esf: '', od_cil: '', od_eixo: '', od_dnp: '', od_alt: '',
    oe_esf: '', oe_cil: '', oe_eixo: '', oe_dnp: '', oe_alt: '',
    adicao: '', lente: '', armacao: '', os_numero: '', observacao: ''
  }
}

function emptyFinanceiro(date = getTodayLocal()) {
  return {
    id: null,
    emissao: date,
    duplicata: '',
...
function statusFinanceiro(item) {
  ...
  const quitado = Number(item.valor_quitado || 0)
  if (quitado <= 0) return { texto: 'Em aberto', classe: 'open' }
  if (quitado < total) return { texto: 'Parcial', classe: 'partial' }
  return { texto: 'Pago', classe: 'paid' }
}
...
export default function App() {
  ...
  const [clienteExames, setClienteExames] = useState([])
  const [clienteFinanceiro, setClienteFinanceiro] = useState([])
  const [exameForm, setExameForm] = useState(emptyExame())
  const [financeiroForm, setFinanceiroForm] = useState(emptyFinanceiro(hoje))
  const [exameSalvando, setExameSalvando] = useState(false)
  const [financeiroSalvando, setFinanceiroSalvando] = useState(false)
  ...
  async function carregarFichaCliente(clienteId) {
    ...
    const [examesResp, financeiroResp] = await Promise.all([
      supabase.from('cliente_exames').select('*').eq('cliente_id', clienteId).order('data_exame', { ascending: false }).order('id', { ascending: false }),
      supabase.from('cliente_financeiro').select('*').eq('cliente_id', clienteId).order('vencimento', { ascending: true }).order('id', { ascending: true })
    ])

    if (examesResp.error || financeiroResp.error) {
      setErro(`Não foi possível carregar a ficha do cliente. ${(examesResp.error || financeiroResp.error).message}`)
      return
    }

    setClienteExames(examesResp.data || [])
    setClienteFinanceiro(financeiroResp.data || [])
  }
  ...
  function novoCliente() {
    ...
    setFinanceiroForm(emptyFinanceiro(selectedDate))
  ...
  function abrirCliente(cliente) {
    ...
    setFinanceiroForm(emptyFinanceiro(selectedDate))
    setShowClienteModal(true)
    carregarFichaCliente(cliente.id)
  }
  ...
  async function salvarExame(e) {
    e.preventDefault()
    setErro('')
    if (!clienteForm.id) { setErro('Salve o cliente antes de lançar exames.'); return }
    setExameSalvando(true)

    const payload = {
      cliente_id: clienteForm.id,
      codempresa: empresaAtual?.codempresa || null,
      data_exame: exameForm.data_exame || getTodayLocal(),
      profissional: exameForm.profissional.trim(),
      receita: exameForm.receita.trim(),
      od_esf: exameForm.od_esf.trim(), od_cil: exameForm.od_cil.trim(), od_eixo: exameForm.od_eixo.trim(), od_dnp: exameForm.od_dnp.trim(), od_alt: exameForm.od_alt.trim(),
      oe_esf: exameForm.oe_esf.trim(), oe_cil: exameForm.oe_cil.trim(), oe_eixo: exameForm.oe_eixo.trim(), oe_dnp: exameForm.oe_dnp.trim(), oe_alt: exameForm.oe_alt.trim(),
      adicao: exameForm.adicao.trim(), lente: exameForm.lente.trim(), armacao: exameForm.armacao.trim(), os_numero: exameForm.os_numero.trim(), observacao: exameForm.observacao.trim()
    }

    const response = exameForm.id
      ? await supabase.from('cliente_exames').update(payload).eq('id', exameForm.id)
      : await supabase.from('cliente_exames').insert([payload])

    if (response.error) {
      setErro(`Não foi possível salvar o exame. ${response.error.message}`)
    ...
    setExameForm(emptyExame())
    setExameSalvando(false)
    await carregarFichaCliente(clienteForm.id)
  }
  ...
  async function salvarFinanceiro(e, lancarCaixa = false) {
    e.preventDefault()
    setErro('')
    if (!clienteForm.id) { setErro('Salve o cliente antes de lançar financeiro.'); return }

    const valorTotal = Number(String(financeiroForm.valor_total || '0').replace(',', '.')) || 0
    const valorQuitado = Number(String(financeiroForm.valor_quitado || '0').replace(',', '.')) || 0
    if (valorTotal <= 0) { setErro('Informe o valor total da ficha financeira.'); return }
    if (valorQuitado > valorTotal) { setErro('Valor quitado não pode ser maior que o valor total.'); return }
    if (lancarCaixa && valorQuitado > 0 && (!aberturaDoDia || fechamentoDoDia)) {
      setErro('Para lançar pagamento no caixa, o caixa do dia precisa estar aberto e não pode estar fechado.')
      return
    }

    setFinanceiroSalvando(true)
    const payload = {
      cliente_id: clienteForm.id,
      codempresa: empresaAtual?.codempresa || null,
      emissao: financeiroForm.emissao || getTodayLocal(),
      ...
      tipo_cobranca: financeiroForm.tipo_cobranca,
      parcela: financeiroForm.parcela.trim(),
      banco: financeiroForm.banco.trim(),
      observacao: financeiroForm.observacao.trim()
    }

    const response = financeiroForm.id
      ? await supabase.from('cliente_financeiro').update(payload).eq('id', financeiroForm.id)
      : await supabase.from('cliente_financeiro').insert([payload])

    if (response.error) {
      setErro(`Não foi possível salvar o financeiro. ${response.error.message}`)
      setFinanceiroSalvando(false)
      return
    }

    if (lancarCaixa && valorQuitado > 0) {
      const { error: caixaError } = await supabase.from('lancamentos_caixa').insert([{
        data: selectedDate,
        descricao: `Pagamento cliente ${clienteForm.nome}`,
        categoria: 'Venda',
        tipo: 'Entrada',
        forma: financeiroForm.forma_pagamento,
        valor: valorQuitado,
        observacao: `Ficha cliente • Duplicata ${financeiroForm.duplicata || '-'} • Parcela ${financeiroForm.parcela || '-'}${financeiroForm.observacao ? ' • ' + financeiroForm.observacao : ''}`,
        responsavel: form.responsavel || 'MEIRE',
        codempresa: empresaAtual.codempresa
      }])
      if (caixaError) {
        setErro(`Financeiro salvo, mas não foi lançado no caixa. ${caixaError.message}`)
        setFinanceiroSalvando(false)
        await carregarFichaCliente(clienteForm.id)
        return
      }
  ...
  function editarFinanceiro(item) {
    setFinanceiroForm({ ...emptyFinanceiro(selectedDate), ...item, valor_total: String(item.valor_total ?? ''), valor_quitado: String(item.valor_quitado ?? '') })
      ...
      <main className="content">
        ...
        {showClienteModal ? (
          <div className="modal-overlay">
            <div className="modal-card client-modal-card ficha-modal-card">
              <div className="modal-head">
                <div>
                  <p className="page-kicker orange-text">Ficha do cliente</p>
                  <h3>{clienteForm.nome || 'Novo cliente'}</h3>
                  <p className="dark-text">Dados, exames de grau e financeiro do cliente em uma tela só.</p>
                </div>
                <button type="button" className="modal-close" onClick={() => setShowClienteModal(false)}>×</button>
              </div>

              <div className="client-summary-grid ficha-summary-grid">
                <div className="daily-metric compact"><small>Código</small><strong>{clienteForm.codigo || 'Auto'}</strong></div>
                <div className="daily-metric compact"><small>Exames</small><strong>{clienteExames.length}</strong></div>
                <div className="daily-metric compact"><small>Total ficha</small><strong>{formatMoney(clienteFinanceiro.reduce((acc, item) => acc + Number(item.valor_total || 0), 0))}</strong></div>
                <div className="daily-metric compact"><small>Pago</small><strong>{formatMoney(clienteFinanceiro.reduce((acc, item) => acc + Number(item.valor_quitado || 0), 0))}</strong></div>
              </div>

              <div className="client-tabs">
                <button type="button" className={`tab-btn ${clienteTab === 'dados' ? 'active' : ''}`} onClick={() => setClienteTab('dados')}>Dados do cliente</button>
                <button type="button" className={`tab-btn ${clienteTab === 'exames' ? 'active' : ''}`} onClick={() => setClienteTab('exames')}>Exames</button>
                <button type="button" className={`tab-btn ${clienteTab === 'financeiro' ? 'active' : ''}`} onClick={() => setClienteTab('financeiro')}>Financeiro</button>
              </div>

              <div className="client-scroll-area ficha-scroll-area">
                {clienteTab === 'dados' ? (
                  ...
                  <div className="ficha-two-columns">
                    <form className="ficha-form-card" onSubmit={salvarExame}>
                      <div className="panel-title-row"><div><h4>{exameForm.id ? 'Editar exame' : 'Novo exame'}</h4><p>Preencha o grau conforme a receita do cliente.</p></div></div>
                      {!clienteForm.id ? <div className="empty-state">Salve o cliente primeiro para liberar os exames.</div> : null}
                      <div className="form-grid">
                        <label className="field"><span>Data do exame</span><input type="date" value={exameForm.data_exame || ''} onChange={(e) => updateExameField('data_exame', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Profissional</span><input value={exameForm.profissional} onChange={(e) => updateExameField('profissional', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Receita / RG</span><input value={exameForm.receita} onChange={(e) => updateExameField('receita', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>OS</span><input value={exameForm.os_numero} onChange={(e) => updateExameField('os_numero', e.target.value)} disabled={!clienteForm.id} /></label>
                      </div>
                      <div className="rx-grid ficha-rx-grid">
                        <div className="rx-card"><h4>OD</h4><div className="form-grid rx-inner-grid"><label className="field"><span>Esf.</span><input value={exameForm.od_esf} onChange={(e) => updateExameField('od_esf', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>Cil.</span><input value={exameForm.od_cil} onChange={(e) => updateExameField('od_cil', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>Eixo</span><input value={exameForm.od_eixo} onChange={(e) => updateExameField('od_eixo', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>DNP</span><input value={exameForm.od_dnp} onChange={(e) => updateExameField('od_dnp', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>Alt.</span><input value={exameForm.od_alt} onChange={(e) => updateExameField('od_alt', e.target.value)} disabled={!clienteForm.id} /></label></div></div>
                        <div className="rx-card"><h4>OE</h4><div className="form-grid rx-inner-grid"><label className="field"><span>Esf.</span><input value={exameForm.oe_esf} onChange={(e) => updateExameField('oe_esf', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>Cil.</span><input value={exameForm.oe_cil} onChange={(e) => updateExameField('oe_cil', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>Eixo</span><input value={exameForm.oe_eixo} onChange={(e) => updateExameField('oe_eixo', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>DNP</span><input value={exameForm.oe_dnp} onChange={(e) => updateExameField('oe_dnp', e.target.value)} disabled={!clienteForm.id} /></label><label className="field"><span>Alt.</span><input value={exameForm.oe_alt} onChange={(e) => updateExameField('oe_alt', e.target.value)} disabled={!clienteForm.id} /></label></div></div>
                      </div>
                      <div className="form-grid">
                        <label className="field"><span>Adição</span><input value={exameForm.adicao} onChange={(e) => updateExameField('adicao', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Lente</span><input value={exameForm.lente} onChange={(e) => updateExameField('lente', e.target.value)} disabled={!clienteForm.id} placeholder="Ex.: Multifocal digital" /></label>
                        <label className="field"><span>Armação</span><input value={exameForm.armacao} onChange={(e) => updateExameField('armacao', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field field-full"><span>Observação</span><textarea rows="3" value={exameForm.observacao} onChange={(e) => updateExameField('observacao', e.target.value)} disabled={!clienteForm.id} /></label>
                      </div>
                      <div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setExameForm(emptyExame())}>Limpar</button><button type="submit" className="primary-inline-btn" disabled={exameSalvando || !clienteForm.id}>{exameSalvando ? 'Salvando...' : 'Salvar exame'}</button></div>
                    </form>

                    <div className="ficha-list-card">
                      <div className="panel-title-row"><div><h4>Histórico de exames</h4><p>Receitas anteriores do cliente.</p></div></div>
                      {clienteExames.length === 0 ? <div className="empty-state">Nenhum exame lançado.</div> : clienteExames.map((item) => (
                        <article className="record-sheet rx-history-card" key={item.id} onClick={() => editarExame(item)}>
                          <div className="record-sheet-head"><div><h4>{formatDate(item.data_exame)} • OS {item.os_numero || '-'}</h4><p>{item.lente || 'Sem lente'} • {item.armacao || 'Sem armação'}</p></div><span className="pill neutral-pill">Editar</span></div>
                          <div className="sheet-grid"><div><small>OD</small><strong>{item.od_esf || '-'} / {item.od_cil || '-'} / {item.od_eixo || '-'}</strong></div><div><small>OE</small><strong>{item.oe_esf || '-'} / {item.oe_cil || '-'} / {item.oe_eixo || '-'}</strong></div><div><small>Ad.</small><strong>{item.adicao || '-'}</strong></div></div>
                          {item.observacao ? <p className="sheet-note">{item.observacao}</p> : null}
                        </article>
                      ))}
                    </div>
                  ...
                  <div className="ficha-two-columns">
                    <form className="ficha-form-card" onSubmit={(e) => salvarFinanceiro(e, false)}>
                      <div className="panel-title-row"><div><h4>{financeiroForm.id ? 'Editar financeiro' : 'Novo lançamento da ficha'}</h4><p>Controle duplicata, vencimento, pagamento e carnê.</p></div></div>
                      {!clienteForm.id ? <div className="empty-state">Salve o cliente primeiro para liberar o financeiro.</div> : null}
                      <div className="form-grid">
                        <label className="field"><span>Emissão</span><input type="date" value={financeiroForm.emissao || ''} onChange={(e) => updateFinanceiroField('emissao', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Duplicata Nº</span><input value={financeiroForm.duplicata} onChange={(e) => updateFinanceiroField('duplicata', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Vencimento</span><input type="date" value={financeiroForm.vencimento || ''} onChange={(e) => updateFinanceiroField('vencimento', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Parcela</span><input value={financeiroForm.parcela} onChange={(e) => updateFinanceiroField('parcela', e.target.value)} placeholder="1/4" disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Valor total</span><input value={financeiroForm.valor_total} onChange={(e) => updateFinanceiroField('valor_total', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Valor quitado</span><input value={financeiroForm.valor_quitado} onChange={(e) => updateFinanceiroField('valor_quitado', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Data pagamento</span><input type="date" value={financeiroForm.data_pagamento || ''} onChange={(e) => updateFinanceiroField('data_pagamento', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field"><span>Forma pagamento</span><select value={financeiroForm.forma_pagamento} onChange={(e) => updateFinanceiroField('forma_pagamento', e.target.value)} disabled={!clienteForm.id}>{formas.map((item) => <option key={item}>{item}</option>)}</select></label>
                        <label className="field"><span>Tipo cobrança</span><select value={financeiroForm.tipo_cobranca} onChange={(e) => updateFinanceiroField('tipo_cobranca', e.target.value)} disabled={!clienteForm.id}><option>Carnê</option><option>Pix</option><option>Dinheiro</option><option>Cartão de débito</option><option>Cartão de crédito</option><option>Transferência</option></select></label>
                        <label className="field"><span>Banco</span><input value={financeiroForm.banco} onChange={(e) => updateFinanceiroField('banco', e.target.value)} disabled={!clienteForm.id} /></label>
                        <label className="field field-full"><span>Observação</span><textarea rows="3" value={financeiroForm.observacao} onChange={(e) => updateFinanceiroField('observacao', e.target.value)} disabled={!clienteForm.id} /></label>
                      </div>
                      <div className="modal-actions"><button type="button" className="secondary-btn" onClick={() => setFinanceiroForm(emptyFinanceiro(selectedDate))}>Limpar</button><button type="submit" className="secondary-btn" disabled={financeiroSalvando || !clienteForm.id}>Salvar ficha</button><button type="button" className="primary-inline-btn" onClick={(e) => salvarFinanceiro(e, true)} disabled={financeiroSalvando || !clienteForm.id}>{financeiroSalvando ? 'Salvando...' : 'Salvar e lançar no caixa'}</button></div>
                    </form>

                    <div className="ficha-list-card">
                      <div className="panel-title-row"><div><h4>Financeiro do cliente</h4><p>Parcelas, duplicatas e pagamentos.</p></div></div>
                      {clienteFinanceiro.length === 0 ? <div className="empty-state">Nenhum lançamento financeiro.</div> : clienteFinanceiro.map((item) => {
                        const status = statusFinanceiro(item)
                        return (
                          <article className={`record-sheet financial-card ${status.classe}`} key={item.id} onClick={() => editarFinanceiro(item)}>
                            <div className="record-sheet-head"><div><h4>Duplicata {item.duplicata || '-'} • {item.parcela || '-'}</h4><p>{item.tipo_cobranca || '-'} • {item.forma_pagamento || '-'}</p></div><span className={`pill finance-${status.classe}`}>{status.texto}</span></div>
                            <div className="sheet-grid"><div><small>Vencimento</small><strong>{formatDate(item.vencimento)}</strong></div><div><small>Total</small><strong>{formatMoney(item.valor_total)}</strong></div><div><small>Quitado</small><strong>{formatMoney(item.valor_quitado)}</strong></div></div>
                            <div className="sheet-grid"><div><small>Pago em</small><strong>{formatDate(item.data_pagamento)}</strong></div><div><small>Banco</small><strong>{item.banco || '-'}</strong></div><div><small>Saldo</small><strong>{formatMoney(Number(item.valor_total || 0) - Number(item.valor_quitado || 0))}</strong></div></div>
                            {item.observacao ? <p className="sheet-note">{item.observacao}</p> : null}
                          </article>
                        )
                      })}
