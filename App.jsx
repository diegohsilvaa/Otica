...
export default function App() {
  ...
  async function salvarCliente(e) {
    ...
    if (clienteSelecionado?.id) {
      response = await supabase.from('clientes').update(payload).eq('id', clienteSelecionado.id)
    } else {
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="menu-card">
          ...
          <button className={`menu-item ${activeSection === 'resumo' ? 'active' : ''}`} onClick={() => setActiveSection('resumo')}>Resumo Gerencial</button>
        </div>
      ...
      <main className="content">
        <header className="topbar">
          <div>
            ...
            <h2>{activeSection === 'clientes' ? 'Cadastro de clientes' : activeSection === 'financeiro' ? 'Controle Financeiro da Ótica' : 'Resumo gerencial da ótica'}</h2>
          </div>
            ...
            <div className="modal-card cliente-modal">
              ...
              <form className="cliente-scroll" onSubmit={salvarCliente}>
                <div className="form-grid cliente-form-grid">
                  <label className="field"><span>Código</span><input value={clienteForm.codigo || 'Automático'} disabled /></label>
