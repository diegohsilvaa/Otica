...
function emptyFinanceiro() {
  return {
    id: null, emissao: getTodayLocal(), duplicata: '', vencimento: '', banco: '', debito: '', data_pagamento: '', valor: '', situacao: 'Em aberto', observacao: ''
  }
}
...
export default function App() {
  ...
  const totalFinanceiroCliente = useMemo(() => financeiroDoCliente.reduce((acc, item) => acc + Number(item.valor || 0), 0), [financeiroDoCliente])
  ...
  async function salvarFinanceiro(e) {
    ...
    setErro('')
    const valor = Number(String(financeiroForm.valor || '0').replace(',', '.'))
    ...
    const payload = {
      cliente_id: clienteId, emissao: financeiroForm.emissao || null, duplicata: financeiroForm.duplicata.trim(), vencimento: financeiroForm.vencimento || null, banco: financeiroForm.banco.trim(), debito: financeiroForm.debito.trim(), data_pagamento: financeiroForm.data_pagamento || null, valor, situacao: financeiroForm.situacao.trim(), observacao: financeiroForm.observacao.trim()
    }
  ...
  function editarFinanceiro(item) {
    setFinanceiroForm({ ...emptyFinanceiro(), ...item, valor: String(item.valor ?? '') })
                ...
                {clienteTab === 'dados' ? (
                  ...
                  <div className="client-scroll-area">
                    <form className="panel nested-panel" onSubmit={salvarFinanceiro}>
                      <div className="panel-head"><div><h3>Financeiro do cliente</h3><p>Parcelas e pagamentos ligados diretamente à ficha.</p></div></div>
                      <div className="form-grid">
                        ...
                        <label className="field"><span>Banco</span><input value={financeiroForm.banco} onChange={(e) => updateFinanceiroField('banco', e.target.value)} /></label>
                        <label className="field"><span>Débito</span><input value={financeiroForm.debito} onChange={(e) => updateFinanceiroField('debito', e.target.value)} /></label>
                        <label className="field"><span>Data pagamento</span><input type="date" value={financeiroForm.data_pagamento} onChange={(e) => updateFinanceiroField('data_pagamento', e.target.value)} /></label>
                        <label className="field"><span>Valor</span><input value={financeiroForm.valor} onChange={(e) => updateFinanceiroField('valor', e.target.value)} /></label>
                        <label className="field"><span>Situação</span><select value={financeiroForm.situacao} onChange={(e) => updateFinanceiroField('situacao', e.target.value)}><option>Em aberto</option><option>Pago</option><option>Parcial</option><option>Cancelado</option></select></label>
                    ...
                    <div className="stack-list">{financeiroDoCliente.length === 0 ? <div className="empty-state">Nenhum lançamento financeiro para este cliente.</div> : financeiroDoCliente.map((item) => <article key={item.id} className="record-sheet"><div className="record-sheet-head"><div><h4>{item.duplicata || 'Lançamento financeiro'}</h4><p>Emissão {formatDate(item.emissao)} • Vencimento {formatDate(item.vencimento)}</p></div><div className="row-actions"><button className="secondary-compact-btn" type="button" onClick={() => editarFinanceiro(item)}>Alterar</button><button className="danger-ghost-btn" type="button" onClick={() => excluirFinanceiro(item.id)}>Excluir</button></div></div><div className="sheet-grid"><div><small>Banco</small><strong>{item.banco || '-'}</strong></div><div><small>Débito</small><strong>{item.debito || '-'}</strong></div><div><small>Pagamento</small><strong>{formatDate(item.data_pagamento)}</strong></div><div><small>Valor</small><strong>{formatMoney(item.valor)}</strong></div><div><small>Situação</small><strong>{item.situacao || '-'}</strong></div></div>{item.observacao ? <p className="sheet-note">{item.observacao}</p> : null}</article>)}</div>
                  </div>
                )}
              </div>
            </div>
