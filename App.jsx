...
export default function App() {
  ...
  async function handleLogin(emailDigitado) {
    ...
    const empresaEncontrada = (data || []).find((empresa) => {
      const ativo = empresa?.ativo === true || empresa?.ativo === 'true' || empresa?.ativo === 1 || empresa?.ativo === '1' || empresa?.ativo === 'S' || empresa?.ativo === 's'
      if (!ativo) return false

      const bruto = String(empresa?.email_acesso || '')
      const emails = bruto
        .split(/[;,
]/)
