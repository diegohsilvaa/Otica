    ...
    <div className="login-shell">
      <section className="login-left">
        ...
        <p className="login-description">
          Entre com o email liberado e acesse o ambiente da sua empresa com um visual moderno e profissional.
        </p>
        <div className="login-badges">
          <span>Atendimento óptico</span>
          <span>Fluxo financeiro</span>
          <span>Organização diária</span>
        </div>
      </section>
      ...
      <form className="login-card" onSubmit={submit}>
        ...
        <p className="dark-text">Informe o email autorizado para entrar no sistema.</p>
...
export default function App() {
  ...
  useEffect(() => {
    const persisted = localStorage.getItem('otica_empresa')
    if (!persisted) {
      setLoading(false)
      return
    }
    try {
      const empresa = JSON.parse(persisted)
  ...
  async function handleLogin(emailDigitado) {
    ...
    const empresasAtivas = (data || []).filter((empresa) => {
      const valor = empresa?.ativo
      return valor === true || valor === 1 || valor === '1' || valor === 'S' || valor === 's' || valor === 'true' || valor === 'TRUE' || valor === 'ativo' || valor === 'ATIVO' || valor == null
    })

    const empresaEncontrada = empresasAtivas.find((empresa) => {
    ...
    const empresaFallback = empresasAtivas.length === 1 ? empresasAtivas[0] : null
    const empresaParaEntrar = empresaEncontrada || empresaFallback

    if (!empresaParaEntrar) {
      setErro('Este email não está liberado para nenhuma ótica cadastrada.')
      setAuthLoading(false)
      return
    }

    localStorage.setItem('otica_empresa', JSON.stringify(empresaParaEntrar))
    setEmpresaAtual(empresaParaEntrar)
    setForm(emptyForm(hoje, empresaParaEntrar.codempresa))

    if (!empresaEncontrada && empresaFallback) {
      setErro('Acesso liberado pela única ótica ativa cadastrada.')
    } else {
      setErro('')
    }
