    ...
    <div className="login-shell">
      <section className="login-left">
        ...
        <p className="login-description">
          Entre com email e senha para acessar o ambiente da sua empresa com um visual moderno e profissional.
        </p>
      ...
      <form className="login-card" onSubmit={submit}>
        ...
        <p className="dark-text">Use o email autorizado e a senha cadastrada no Supabase Auth.</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      ...
      if (error) {
        setErro(`Não foi possível validar a sessão. ${error.message}`)
        setLoading(false)
        return
      }
      ...
      await carregarEmpresaPorEmail(userEmail)
    }
  ...
  async function carregarEmpresaPorEmail(emailDigitado) {
    ...
    const { data: acesso, error: acessoError } = await supabase
      ...
      .select('email, ativo, perfil, codempresa, empresas:codempresa (codempresa, nome, email_acesso, ativo)')
      .eq('email', emailNormalizado)
      .eq('ativo', true)
      .maybeSingle()

    if (!acessoError && acesso?.empresas?.codempresa) {
      const empresa = Array.isArray(acesso.empresas) ? acesso.empresas[0] : acesso.empresas
    ...
    if (!empresaEncontrada) {
      setEmpresaAtual(null)
      setErro('Este usuário autenticado não está vinculado a nenhuma ótica cadastrada.')
      setLoading(false)
      return null
    }
  ...
  async function handleLogin(emailDigitado, senhaDigitada) {
    ...
    if (!senha) {
      setErro('Informe a senha para entrar no sistema.')
      return
    }
